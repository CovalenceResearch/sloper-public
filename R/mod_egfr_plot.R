...
mod_egfr_plot_server <- function(id, data, base_egfr, dial_threshold,
  ...
  plot_data <- reactive({
    ...
    dplyr::select(dplyr::all_of(c("tx",
                                  "stats",
                                  "time_value",
                                  "egfr_value",
                                  "trial_data"))) |> 
    tidyr::pivot_wider(names_from = "stats",
                       names_prefix = "egfr_",
  ...
  output$egfr_plot <- highcharter::renderHighchart({
    highcharter::highchart() |> 
      highcharter::hc_add_series(plot_data(),
                                 "line",
                                 highcharter::hcaes(x = .data$time_value,
                                                    y = .data$egfr_central,
                                                    group = .data$tx),
                                 name = paste0("Central: ",
                                               c(tx1(), tx2())),
      ...
      highcharter::hc_xAxis(
        ...
        plotLines = list(
          list(value = acute_year(),
               label = list(text = "Acute phase ends",
                            rotation = 0)),
          list(value = trial_end_year(),
               label = list(text = "Trial ends",
                            rotation = 0,
      ...
      highcharter::hc_yAxis(
        ...
        plotLines = list(
          list(value = dial_threshold(),
               label = list(text = "Dialysis initiation threshold",
                            align = "center"))),
        min = 0
      ) |> 
      highcharter::hc_plotOptions(series = list(connectNulls = TRUE,
                                                lineMarkers = TRUE)) |> 
